...
<div class="flex min-h-full flex-col justify-center py-12 sm:px-6 lg:px-8">
    ...
    <div id="formContainer" class="mt-24 sm:mx-auto sm:w-full sm:max-w-md">
        <div class="flex justify-between text-lg mb-4 text-gray-50">
            <h1 class={!isSigningUp ? 'text-indigo-300' : ''}>Sign in</h1>
            <h1 class={isSigningUp ? 'text-indigo-300' : ''}>Sign up</h1>
        </div>
        <div class="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
            ...
            {:else}
                <form class="space-y-6" on:submit|preventDefault={handleMagicLinkLogin}>
                    <div>
                        <label for="email" class="block text-sm font-medium text-gray-700"
                            >Email address</label
                        >
